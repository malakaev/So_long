...
void	ft_put_info_on_tab(t_param *prm, t_map *map)
{
	...
	mlx_string_put(prm->mlx, prm->win, 60, DTXT, YELLOW, prm->num_str);
	if (prm->win_width > 350)
	{
		ft_int_to_char(prm->collected, prm);
		mlx_string_put(prm->mlx, prm->win, 210, DTXT, GREEN, "Collected:");
		mlx_string_put(prm->mlx, prm->win, 300, DTXT, GREEN, prm->num_str);
	}
	if (prm->win_width > 440)
	{
		ft_int_to_char(prm->coll_sum, prm);
		mlx_string_put(prm->mlx, prm->win, 270 + map->sprt_size,
		DTXT, GREEN, " of ");
		mlx_string_put(prm->mlx, prm->win, 270 + map->sprt_size + 40, \
